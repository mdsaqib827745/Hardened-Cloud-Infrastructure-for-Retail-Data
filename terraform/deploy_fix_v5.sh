#!/bin/bash
# DEPLOY FIX V5 - RFC 8187 HEADERS & CACHE CONTROL
set -e

# 1. Update app.py with v5 logic
...
# 2. Reinforce Nginx Configuration for Header Transparent Passthrough
...
server {
    ...
    location / {
        ...
        # Explicitly pass the specific headers
        proxy_pass_header Content-Disposition;
        proxy_pass_header Access-Control-Expose-Headers;
        proxy_set_header X-Content-Type-Options nosniff;
        
        # Disable buffering to allow immediate header processing
        proxy_buffering off;
...
sleep 2
echo "FIX V5 APPLIED: RFC_8187_HEADERS, CACHE_DISABLED, NGINX_TRANSPARENT"
